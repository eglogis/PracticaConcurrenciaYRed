...
extension TopicsCoordinator: TopicsCoordinatorDelegate {
    func didSelect(topic: Topic) {
        let topicDetailCoordinator = TopicDetailCoordinator(presenter: presenter, topicDetailDataManager: topicDetailDataManager, topicId: topic.id)
        addChildCoordinator(topicDetailCoordinator)
        topicDetailCoordinator.start()

        topicDetailCoordinator.onBackTapped = { [weak self] in
            guard let self = self else { return }
            topicDetailCoordinator.finish()
            self.removeChildCoordinator(topicDetailCoordinator)
        }

        topicDetailCoordinator.onTopicDelete = { [weak self] in
            guard let self = self else { return }
            topicDetailCoordinator.finish()
            self.removeChildCoordinator(topicDetailCoordinator)
            self.topicsViewModel?.updateTopics()
        }
    }
